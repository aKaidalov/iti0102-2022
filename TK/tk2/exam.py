...
def lucky_guess(n: int) -> bool:
    ...
    * smaller than 0 and does not contain number 5 or 6
    ...
    """
    if (n == 1 or n == 3 or n == 7) or (-6 <= n <= 121 and n % 13 == 0):
        return True
    elif n < 0:
        n = str(n)
        if "5" in n or "6" in n:
...
def max_duplicate(nums: list) -> int | None:
    ...
    """
    nums_in_set = list(set(nums))
    new_quantity = 0
    largest_element = 0
    new_element = 0
    for element in nums_in_set:
        quantity = nums.count(element)
        if quantity > new_quantity and quantity > 1:
