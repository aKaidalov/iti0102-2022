...
def filter_by_course(student_list: list, course: str) -> list:
    ...
    """
    return list(filter(lambda x: course in x.courses, student_list))
...
def is_failing(student: Student) -> bool:
    ...
    """
    res = True if student.grade < 1 else False
    return res
...
def succeeding_students(student_list: list) -> list:
    ...
    """
    return list(filter(lambda x: x if x.grade >= 1 else None, student_list))
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    """
    not_failing_school = succeeding_students(student_list)
    return list(sorted(not_failing_school, key=lambda x: (-x.grade, x.name)))
