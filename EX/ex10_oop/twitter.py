...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    tweets_growth = list(map(lambda x: round(x.retweets / x.time, 2), tweets))
    fastest = max(tweets_growth)
    return tweets[tweets_growth.index(fastest)]
...
def sort_by_popularity(tweets: list) -> list:
    ...
    """
    return list(sorted(tweets, key=lambda x: (-x.retweets, x.time)))
