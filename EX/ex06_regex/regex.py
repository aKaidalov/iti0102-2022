...
def find_words(text: str) -> list:
    ...
    """
    return re.findall(r"[A-ZÕÜÄÖ][a-zõüäö]+", text)
...
def find_words_with_vowels(text: str) -> list:
    ...
    """
    # list = []
    # pattern = r"[AOEUIÜÕÖÄ][a-zõüäö]+"
    # words = re.finditer(pattern, text)
    # for word in words:
    #     list.append(word.group())
    # return list

    return re.findall(r"[AOEUIÜÕÖÄ][a-zõüäö]+", text)
...
def find_words_from_sentence(sentence: str) -> list:
    ...
    """
    return re.findall(r"[A-ZÕÜÄÖa-zõüäö\d*]\w*", sentence)
...
def find_words_from_sentences_only(text: str) -> list:
    ...
    """
    sentence = "".join(find_sentences(text))
    return find_words_from_sentence(sentence)
...
def find_phone_numbers(text: str) -> dict:
    ...
    for phone in re.finditer(r"(\+\d{3})?\s?(\d{7,8})", text):
        if not phone.group(1):
            ...
            dic[phone.group(1)].append(phone.group(2))
    return dic
...
if __name__ == '__main__':
    ...
    print(find_words_from_sentences_only(
        'See on esimene - ä lause. See, on teine: lause! see ei ole lause. Aga kas see 1994 on? jah, oli.'))  # ['See', 'on', 'esimene', 'ä', 'lause', 'See', 'on', 'teine', 'lause', 'Aga', 'kas', 'see', 'on']
